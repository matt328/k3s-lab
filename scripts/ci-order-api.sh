...
case "${mode}" in
  main)
    version="${ORDER_API_RELEASE_VERSION:-${base_version}}"
    ;;
  feature)
    ...
    version="${ORDER_API_PRERELEASE_VERSION:-${base_version}-${branch_slug}.${short_sha}}"
    ;;
esac

password="$(tr -d '\n' <"${token_file}")"
artifact_path="dev/teeter/demos/apis/order-api-spec/${version}/order-api-spec-${version}.yaml"

echo "Order API CI simulation"
echo "  mode:       ${mode}"
...
cat <<EOF
Published Order API artifact:
  dev.teeter.demos.apis:order-api-spec:${version}@yaml
